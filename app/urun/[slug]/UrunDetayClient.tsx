...
export default function UrunDetayClient({ initialUrun = null }: { initialUrun?: UrunDetay | null }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setAboneToken(t);
      if (!t) { setAboneDurum("yok"); return; }
      try {
        const res = await fetch("/api/abonelik", { headers: { Authorization: `Bearer ${t}` } });
        const d = await res.json();
        const mevcut = (d.abonelikler || []).find((a: { urun_id: number; cadence_gun: number }) => a.urun_id === urun.id);
        if (iptal) return;
        if (mevcut) { setAboneDurum("var"); setAboneCadenceMevcut(mevcut.cadence_gun); }
        else setAboneDurum("yok");
      } catch { if (!iptal) setAboneDurum("yok"); }
    })();
    return () => { iptal = true; };
  }, [urun]);
          ...
          <div style={{ marginTop: 20, background: "white", borderRadius: 16, padding: "16px 18px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[
                ...
                { icon: "⚡", text: <><strong>Aynı Gün Kargo</strong> — Saat 13:00&apos;a kadar verilen siparişler bugün kargoya verilir</> },
                // İzmir elden teslimat duyurusu — hizmet kapalıyken gösterilmez.
                ...(ELDEN_TESLIMAT.ACIK ? [{ icon: "🛵", text: <><strong>İzmir&apos;e Aynı Gün ELDEN Teslimat</strong> — 12:00&apos;a kadar siparişte bugün 15:00–18:00 kapında (9 merkez ilçe, min ₺1.000 sepet, kapıda nakit)</> }] : []),
          ...
          <div style={{ background: "white", borderRadius: 24, padding: "24px" }}>
            {urun.aciklama ? (
              <div style={{ fontSize: 15, color: "#5C3D2E", opacity: 0.75, lineHeight: 1.8 }}
                dangerouslySetInnerHTML={{ __html: urun.aciklama }} />
            ) : (
