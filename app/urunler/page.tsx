...
export default function Urunler() {
  ...
  const [filtrePaneli, setFiltrePaneli] = useState(true);
  ...
  const FilterSelect = ({ label, value, onChange, options }: any) => (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: "#5C3D2E", opacity: 0.5, textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 8 }}>{label}</div>
      <select value={value} onChange={e => onChange(e.target.value)}
        style={{ width: "100%", padding: "10px 14px", border: `2px solid ${value ? "#E8845A" : "#E8D5B7"}`, borderRadius: 12, fontSize: 13, outline: "none", fontFamily: "inherit", background: "white", color: "#5C3D2E", cursor: "pointer" }}>
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#FDF6EE", fontFamily: "sans-serif" }}>
      ...
      <header style={{ background: "white", padding: "16px 48px", borderBottom: "1px solid #E8D5B7", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <a href="/sepet" style={{ background: "#5C3D2E", color: "white", padding: "10px 20px", borderRadius: 50, textDecoration: "none", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
          🛒 Sepet {totalItems > 0 && <span style={{ background: "#E8845A", borderRadius: 50, padding: "1px 7px", fontSize: 11 }}>{totalItems}</span>}
        </a>
      </header>

      {/* Breadcrumb */}
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "14px 24px", fontSize: 13, color: "#5C3D2E", opacity: 0.6 }}>
        <a href="/" style={{ color: "#E8845A", textDecoration: "none" }}>Ana Sayfa</a> / Tüm Ürünler
      </div>

      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 24px 48px", display: "grid", gridTemplateColumns: filtrePaneli ? "280px 1fr" : "0px 1fr", gap: filtrePaneli ? 24 : 0, transition: "all .3s" }}>
          ...
          <div>
            <div style={{ background: "white", borderRadius: 20, padding: "20px", marginBottom: 16, boxShadow: "0 4px 16px rgba(92,61,46,0.06)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <div style={{ fontFamily: "Georgia, serif", fontSize: 16, fontWeight: 700, color: "#5C3D2E" }}>🔧 Filtreler {aktifFiltreSayisi > 0 && <span style={{ background: "#E8845A", color: "white", borderRadius: 50, fontSize: 11, padding: "2px 7px", marginLeft: 6 }}>{aktifFiltreSayisi}</span>}</div>
                {aktifFiltreSayisi > 0 && (
              ...
              {/* Arama */}
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#5C3D2E", opacity: 0.5, textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 8 }}>🔍 Ara</div>
                <input value={aramaMetni} onChange={e => setAramaMetni(e.target.value)}
                  placeholder="Ürün ara..." style={{ width: "100%", padding: "10px 14px", border: "2px solid #E8D5B7", borderRadius: 12, fontSize: 13, outline: "none", fontFamily: "inherit", boxSizing: "border-box" as const }} />
              </div>

              {/* Stok */}
              <div style={{ marginBottom: 16, display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={() => setSadeceStoktaki(!sadeceStoktaki)}>
              ...
              {seciliKategori && altKategoriMap[anaKategoriler.find(k => k.slug === seciliKategori)?.id || 0]?.length > 0 && (
                <FilterSelect label="📂 Alt Kategori" value={seciliKategori} onChange={setSeciliKategori}
                  options={altKategoriMap[anaKategoriler.find(k => k.slug === seciliKategori)?.id || 0]?.map((k: any) => ({ value: k.slug, label: k.ad }))} />
              )}
              ...
              {/* Fiyat Aralığı */}
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#5C3D2E", opacity: 0.5, textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 8 }}>💰 Fiyat Aralığı (₺)</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <input type="number" value={minFiyat} onChange={e => setMinFiyat(e.target.value)}
                    placeholder="Min" style={{ padding: "10px 12px", border: "2px solid #E8D5B7", borderRadius: 12, fontSize: 13, outline: "none", fontFamily: "inherit", width: "100%", boxSizing: "border-box" as const }} />
                  <input type="number" value={maxFiyat} onChange={e => setMaxFiyat(e.target.value)}
                    placeholder="Max" style={{ padding: "10px 12px", border: "2px solid #E8D5B7", borderRadius: 12, fontSize: 13, outline: "none", fontFamily: "inherit", width: "100%", boxSizing: "border-box" as const }} />
                </div>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <button onClick={() => setFiltrePaneli(!filtrePaneli)}
                style={{ background: "white", border: "2px solid #E8D5B7", borderRadius: 12, padding: "10px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer", color: "#5C3D2E", display: "flex", alignItems: "center", gap: 6 }}>
                {filtrePaneli ? "◀ Filtreleri Gizle" : "▶ Filtreleri Göster"}
                {aktifFiltreSayisi > 0 && <span style={{ background: "#E8845A", color: "white", borderRadius: 50, fontSize: 11, padding: "1px 6px" }}>{aktifFiltreSayisi}</span>}
              </button>
              <div style={{ fontFamily: "Georgia, serif", fontSize: 20, fontWeight: 700, color: "#5C3D2E" }}>
                Tüm Ürünler
                <span style={{ fontSize: 13, fontWeight: 400, opacity: 0.5, marginLeft: 8 }}>({filtrelenmis.length} ürün)</span>
              </div>
            </div>
            <select value={siralama} onChange={e => setSiralama(e.target.value)}
              style={{ padding: "10px 16px", border: "2px solid #E8D5B7", borderRadius: 12, fontSize: 13, outline: "none", fontFamily: "inherit", background: "white", color: "#5C3D2E", cursor: "pointer" }}>
          ...
          {yukleniyor ? (
            ...
            <div style={{ textAlign: "center", padding: "80px 0", background: "white", borderRadius: 24 }}>
              <div style={{ fontSize: 48, marginBottom: 16 }}>😢</div>
              ...
              {sayfadakiUrunler.map((urun, i) => (
                <div key={i} style={{ background: "white", borderRadius: 20, overflow: "hidden", transition: "transform .2s, box-shadow .2s" }}
                  ...
                  <a href={`/urun/${urun.slug}`} style={{ textDecoration: "none", display: "block" }}>
                    <div style={{ height: 160, background: "#FDF6EE", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                      {urun.resim_url ? (
                      ...
                      {urun.stok === 0 && (
                        <div style={{ position: "absolute", inset: 0, background: "rgba(255,255,255,0.75)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <span style={{ background: "#5C3D2E", color: "white", fontSize: 11, fontWeight: 700, padding: "6px 12px", borderRadius: 50 }}>Stokta Yok</span>
                        </div>
                      )}
                      {urun.stok > 0 && urun.stok <= 5 && (
                        <span style={{ position: "absolute", top: 8, left: 8, background: "#E8845A", color: "white", fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 50 }}>Son {urun.stok}!</span>
                      )}
                    </div>
                    <div style={{ padding: "12px 14px 8px" }}>
                      {urun.markalar && <div style={{ fontSize: 10, fontWeight: 700, color: "#8BAF8E", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 4 }}>{urun.markalar.ad}</div>}
                      <div style={{ fontFamily: "Georgia, serif", fontSize: 14, fontWeight: 700, color: "#5C3D2E", lineHeight: 1.3, marginBottom: 4 }}>{urun.ad.substring(0, 55)}{urun.ad.length > 55 ? "..." : ""}</div>
                    </div>
                  </a>
          ...
          {toplamSayfa > 1 && (
            <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 32, flexWrap: "wrap" }}>
              <button onClick={() => setSayfa(1)} disabled={sayfa === 1}
                style={{ padding: "10px 16px", border: "2px solid #E8D5B7", borderRadius: 12, background: "white", cursor: sayfa === 1 ? "not-allowed" : "pointer", color: "#5C3D2E", fontWeight: 600, opacity: sayfa === 1 ? 0.4 : 1 }}>«</button>
              <button onClick={() => setSayfa(s => Math.max(1, s - 1))} disabled={sayfa === 1}
                style={{ padding: "10px 16px", border: "2px solid #E8D5B7", borderRadius: 12, background: "white", cursor: sayfa === 1 ? "not-allowed" : "pointer", color: "#5C3D2E", fontWeight: 600, opacity: sayfa === 1 ? 0.4 : 1 }}>← Önceki</button>
              {Array.from({ length: Math.min(5, toplamSayfa) }, (_, i) => {
                ...
                return (
                  <button key={p} onClick={() => setSayfa(p)}
                    style={{ padding: "10px 16px", border: `2px solid ${sayfa === p ? "#E8845A" : "#E8D5B7"}`, borderRadius: 12, background: sayfa === p ? "#E8845A" : "white", cursor: "pointer", color: sayfa === p ? "white" : "#5C3D2E", fontWeight: 700 }}>
                    {p}
                  </button>
                );
              })}
              <button onClick={() => setSayfa(s => Math.min(toplamSayfa, s + 1))} disabled={sayfa === toplamSayfa}
                style={{ padding: "10px 16px", border: "2px solid #E8D5B7", borderRadius: 12, background: "white", cursor: sayfa === toplamSayfa ? "not-allowed" : "pointer", color: "#5C3D2E", fontWeight: 600, opacity: sayfa === toplamSayfa ? 0.4 : 1 }}>Sonraki →</button>
              <button onClick={() => setSayfa(toplamSayfa)} disabled={sayfa === toplamSayfa}
                style={{ padding: "10px 16px", border: "2px solid #E8D5B7", borderRadius: 12, background: "white", cursor: sayfa === toplamSayfa ? "not-allowed" : "pointer", color: "#5C3D2E", fontWeight: 600, opacity: sayfa === toplamSayfa ? 0.4 : 1 }}>»</button>
            </div>
          )}
        </div>
      </div>
