...
export default function Admin() {
  ...
  const [istatistikler, setIstatistikler] = useState({ urunler: 0, siparisler: 0, kategoriler: 0, bekleyenSoru: 0 });
  ...
  const urunleriYukle = useCallback(async (sayfa = 0, arama = "", filtre = { kategori: "", marka: "", stok: "", durum: "" }) => {
    ...
    let q = supabase.from("urunler").select("*, kategoriler(ad,id), markalar(ad,id)", { count: "exact" });
  ...
  const istatistikleriYukle = async () => {
    const [{ count: uc }, { count: sc }, { count: kc }, { count: bc }] = await Promise.all([
    ...
    setIstatistikler({ urunler: uc || 0, siparisler: sc || 0, kategoriler: kc || 0, bekleyenSoru: bc || 0 });
  };
  ...
  // Filtre değişince
  const filtreUygula = (yeniFiltre: typeof filtreler) => {
    setFiltreler(yeniFiltre); setSayfaNo(0);
  };

  const filtreleriTemizle = () => {
    setAramaMetni(""); setFiltreler({ kategori: "", marka: "", stok: "", durum: "" }); setSayfaNo(0);
    if (aramaRef.current) aramaRef.current.value = "";
  };
  ...
  // Inline (hızlı) düzenleme kaydet
  const inlineKaydet = async () => {
    if (!inlineEdit) return;
    const guncelleme: any = {};
    if (inlineEdit.alan === "fiyat") guncelleme.fiyat = parseFloat(inlineEdit.deger);
    if (inlineEdit.alan === "indirimli_fiyat") guncelleme.indirimli_fiyat = inlineEdit.deger ? parseFloat(inlineEdit.deger) : null;
    if (inlineEdit.alan === "stok") guncelleme.stok = parseInt(inlineEdit.deger);
    await supabase.from("urunler").update(guncelleme).eq("id", inlineEdit.id);
    setInlineEdit(null);
    urunleriYukle(sayfaNo, aramaMetni, filtreler);
    goster("✅ Güncellendi");
  };
  ...
  const topluIslemUygula = async () => {
    ...
    if (topluIslem.tip === "fiyat_yuzde" && topluIslem.deger) {
      ...
      for (const id of seciliUrunler) {
        ...
        if (u) await supabase.from("urunler").update({ fiyat: Math.round(u.fiyat * (1 + yuzde) * 100) / 100 }).eq("id", id);
      }
    } else if (topluIslem.tip === "indirim_yuzde" && topluIslem.deger) {
      const yuzde = parseFloat(topluIslem.deger) / 100;
      for (const id of seciliUrunler) {
        const u = urunler.find(u => u.id === id);
        if (u) await supabase.from("urunler").update({ indirimli_fiyat: Math.round(u.fiyat * (1 - yuzde) * 100) / 100 }).eq("id", id);
      }
    } else if (topluIslem.tip === "fiyat_tl" && topluIslem.deger) {
      const miktar = parseFloat(topluIslem.deger);
      for (const id of seciliUrunler) {
        const u = urunler.find(u => u.id === id);
        if (u) await supabase.from("urunler").update({ fiyat: Math.max(0, Math.round((u.fiyat + miktar) * 100) / 100) }).eq("id", id);
      }
    ...
    const sayac = seciliUrunler.length;
    setSeciliUrunler([]);
    urunleriYukle(sayfaNo, aramaMetni, filtreler);
    istatistikleriYukle();
    setYukleniyor(false);
    goster(`✅ ${sayac} ürüne işlem uygulandı`);
  };
    ...
    <main style={{ minHeight: "100vh", background: "#FDF6EE", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "sans-serif" }}>
      <div style={{ background: "white", borderRadius: 24, padding: "48px 40px", maxWidth: 380, width: "100%", boxShadow: "0 20px 60px rgba(92,61,46,0.1)", textAlign: "center" }}>
        ...
        <input
          ...
          placeholder="Şifre"
          style={{ ...s, marginBottom: 10, textAlign: "center", fontSize: 18, letterSpacing: 4 }}
        />
  ...
  const menuler = [
    ...
    { id: "urunler", icon: "📦", ad: "Ürünler", badge: 0 },
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#F0EBE3", fontFamily: "sans-serif", display: "flex" }}>
      ...
      {/* BİLDİRİM */}
      {bildirim && (
        <div style={{ position: "fixed", top: 24, right: 24, background: bildirim.startsWith("❌") ? "#C62828" : "#2C1A0E", color: "white", padding: "14px 22px", borderRadius: 14, fontSize: 14, fontWeight: 600, zIndex: 9999, boxShadow: "0 8px 32px rgba(0,0,0,0.25)", display: "flex", alignItems: "center", gap: 8 }}>
          {bildirim}
        </div>
      )}
        ...
        <div onClick={e => { if (e.target === e.currentTarget) setDuzenleUrun(null); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: "white", borderRadius: 20, padding: 28, width: "100%", maxWidth: 720, maxHeight: "92vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div>
                <h2 style={{ fontFamily: "Georgia,serif", fontSize: 18, fontWeight: 700, color: "#5C3D2E", margin: 0 }}>✏️ Tam Düzenleme</h2>
            ...
            {duzenleUrun.resim_url && (
              <div style={{ marginBottom: 16, padding: 12, background: "#FDF6EE", borderRadius: 12, display: "flex", alignItems: "center", gap: 12 }}>
                <img src={duzenleUrun.resim_url} alt="" style={{ width: 72, height: 72, objectFit: "contain", borderRadius: 10, background: "white", padding: 6 }} />
                <div style={{ fontSize: 12, color: "#5C3D2E", opacity: 0.6 }}>Mevcut resim — URL'yi değiştirerek güncelleyebilirsiniz</div>
              </div>
            )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: "#5C3D2E", opacity: 0.7, display: "block", marginBottom: 5 }}>İndirimli Fiyat (₺) <span style={{ opacity: 0.5 }}>— boş = indirim yok</span></label>
                <input type="number" step="0.01" value={duzenleUrun.indirimli_fiyat || ""} onChange={e => setDuzenleUrun({ ...duzenleUrun, indirimli_fiyat: e.target.value })} style={s} placeholder="Boş bırakın" />
              </div>
              <div>
              ...
              <div>
                ...
                <select value={duzenleUrun.aktif ? "1" : "0"} onChange={e => setDuzenleUrun({ ...duzenleUrun, aktif: e.target.value === "1" })} style={s}>
                  <option value="1">✅ Aktif (Satışta)</option>
              ...
              <div style={{ gridColumn: "1/-1" }}>
                ...
                <input value={duzenleUrun.resim_url || ""} onChange={e => setDuzenleUrun({ ...duzenleUrun, resim_url: e.target.value })} style={s} placeholder="https://www.sepetmama.com/wp-content/..." />
              </div>
              <div>
            ...
            <div style={{ marginTop: 12, marginBottom: 20 }}>
              <label style={{ fontSize: 12, fontWeight: 700, color: "#5C3D2E", opacity: 0.7, display: "block", marginBottom: 5 }}>Uzun Açıklama (HTML destekler)</label>
              <textarea value={duzenleUrun.aciklama || ""} onChange={e => setDuzenleUrun({ ...duzenleUrun, aciklama: e.target.value })}
                rows={5} style={{ ...s, resize: "vertical" as const }} />
            </div>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, marginBottom: 24 }}>
              {[
                ...
                { icon: "📁", ad: "Kategori", deger: istatistikler.kategoriler, renk: "#5C3D2E", sayfa: "kategoriler" },
                { icon: "📝", ad: "Bekleyen Soru", deger: istatistikler.bekleyenSoru, renk: "#9C27B0", sayfa: "blog" },
              ].map((k, i) => (
                <div key={i} onClick={() => setAktifSayfa(k.sayfa)} style={{ background: "white", borderRadius: 18, padding: 20, boxShadow: "0 4px 16px rgba(92,61,46,0.06)", cursor: "pointer", transition: "transform .15s" }}
            ...
            <div style={{ background: "white", borderRadius: 18, padding: 24, boxShadow: "0 4px 16px rgba(92,61,46,0.06)" }}>
              <h3 style={{ fontFamily: "Georgia,serif", fontSize: 16, fontWeight: 700, color: "#2C1A0E", marginBottom: 16 }}>Hızlı Erişim</h3>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {menuler.filter(m => m.id !== "dashboard").map((m, i) => (
                  <button key={i} onClick={() => setAktifSayfa(m.id)} style={{ background: "#FDF6EE", border: "2px solid #E8D5B7", borderRadius: 12, padding: "10px 16px", fontSize: 13, fontWeight: 600, color: "#5C3D2E", cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 7 }}>
                    {m.icon} {m.ad} {m.badge && m.badge > 0 ? <span style={{ background: "#E8845A", color: "white", borderRadius: 50, fontSize: 10, padding: "1px 6px" }}>{m.badge}</span> : null}
                  </button>
                ))}
              </div>
            </div>
        ...
        {aktifSayfa === "urunler" && (
          <div>
            ...
            {yeniUrunAcik && (
              <div style={{ background: "white", borderRadius: 18, padding: 22, marginBottom: 16, boxShadow: "0 4px 16px rgba(92,61,46,0.06)", border: "2px solid #E8845A" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "#5C3D2E", opacity: 0.6, display: "block", marginBottom: 4 }}>ÜRÜN ADI *</label>
                    <input type="text" autoComplete="off" placeholder="Ürün adını yazın..." value={yeniUrun.ad} onChange={e => setYeniUrun({ ...yeniUrun, ad: e.target.value })} style={s} /></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "#5C3D2E", opacity: 0.6, display: "block", marginBottom: 4 }}>FİYAT ₺ *</label>
                    <input type="number" step="0.01" placeholder="0.00" value={yeniUrun.fiyat} onChange={e => setYeniUrun({ ...yeniUrun, fiyat: e.target.value })} style={s} /></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "#5C3D2E", opacity: 0.6, display: "block", marginBottom: 4 }}>İNDİRİMLİ ₺</label>
                    <input type="number" step="0.01" placeholder="Boş bırakın" value={yeniUrun.indirimli_fiyat} onChange={e => setYeniUrun({ ...yeniUrun, indirimli_fiyat: e.target.value })} style={s} /></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "#5C3D2E", opacity: 0.6, display: "block", marginBottom: 4 }}>STOK</label>
                    <input type="number" placeholder="0" value={yeniUrun.stok} onChange={e => setYeniUrun({ ...yeniUrun, stok: e.target.value })} style={s} /></div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "#5C3D2E", opacity: 0.6, display: "block", marginBottom: 4 }}>RESİM URL</label>
                    <input type="text" autoComplete="off" placeholder="https://..." value={yeniUrun.resim_url} onChange={e => setYeniUrun({ ...yeniUrun, resim_url: e.target.value })} style={s} /></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "#5C3D2E", opacity: 0.6, display: "block", marginBottom: 4 }}>KATEGORİ</label>
                    <select value={yeniUrun.kategori_id} onChange={e => setYeniUrun({ ...yeniUrun, kategori_id: e.target.value })} style={s}><option value="">Seçin</option>{kategoriler.map(k => <option key={k.id} value={k.id}>{k.ad}</option>)}</select></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "#5C3D2E", opacity: 0.6, display: "block", marginBottom: 4 }}>MARKA</label>
                    <select value={yeniUrun.marka_id} onChange={e => setYeniUrun({ ...yeniUrun, marka_id: e.target.value })} style={s}><option value="">Seçin</option>{markalar.map(m => <option key={m.id} value={m.id}>{m.ad}</option>)}</select></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "#5C3D2E", opacity: 0.6, display: "block", marginBottom: 4 }}>ETİKET</label>
                    <select value={yeniUrun.etiket} onChange={e => setYeniUrun({ ...yeniUrun, etiket: e.target.value })} style={s}><option value="">Yok</option><option value="yeni">🆕 Yeni</option><option value="indirim">💥 İndirim</option><option value="cok-satan">⭐ Çok Satan</option><option value="kampanya">🏷️ Kampanya</option><option value="son-stok">⚠️ Son Stok</option></select></div>
                </div>
            ...
            <div style={{ background: "white", borderRadius: 18, padding: "14px 18px", marginBottom: 12, boxShadow: "0 4px 16px rgba(92,61,46,0.06)", display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              {/* ARAMA — type="search" kullan, password değil */}
              <div style={{ position: "relative", flex: 1, minWidth: 200 }}>
                ...
                />
                {aramaMetni && (
                  <button onClick={() => setAramaMetni("")} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", fontSize: 16, cursor: "pointer", color: "#999", padding: 0 }}>✕</button>
                )}
              </div>
              ...
              {filtrelerAktif && (
                <button onClick={filtreleriTemizle} style={{ ...btn("#888"), padding: "9px 14px", fontSize: 12 }}>✕ Filtreleri Temizle</button>
              )}
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                ...
                {(topluIslem.tip === "fiyat_yuzde" || topluIslem.tip === "indirim_yuzde") && (
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <input type="number" placeholder="% ör: 10 veya -10"
                      value={topluIslem.deger} onChange={e => setTopluIslem({ ...topluIslem, deger: e.target.value })}
                      style={{ ...fltSelect, width: 160 }} />
                    <span style={{ fontSize: 12, opacity: 0.6 }}>%</span>
                  </div>
                )}

                {topluIslem.tip === "fiyat_tl" && (
                  <input type="number" placeholder="₺ ör: 50 veya -50"
                    value={topluIslem.deger} onChange={e => setTopluIslem({ ...topluIslem, deger: e.target.value })}
                    style={{ ...fltSelect, width: 160 }} />
                )}

                {topluIslem.tip === "stok_sifirla" && (
                  <input type="number" placeholder="Stok adedi"
                    value={topluIslem.deger} onChange={e => setTopluIslem({ ...topluIslem, deger: e.target.value })}
                    style={{ ...fltSelect, width: 130 }} />
                )}

                {topluIslem.tip === "etiket" && (
                  <select value={topluIslem.etiket} onChange={e => setTopluIslem({ ...topluIslem, etiket: e.target.value })} style={fltSelect}>
                ...
                {seciliUrunler.length > 0 && (
                  <button onClick={() => setSeciliUrunler([])} style={{ ...btn("#888"), padding: "9px 14px", fontSize: 12 }}>✕ Seçimi Temizle</button>
                )}
              </div>
            </div>
            ...
            <div style={{ background: "white", borderRadius: 18, boxShadow: "0 4px 16px rgba(92,61,46,0.06)", overflow: "hidden" }}>
              {/* Tablo başlığı */}
              <div style={{ padding: "12px 18px", borderBottom: "1px solid #F0E8E0", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => { setSayfaNo(0); urunleriYukle(0, aramaMetni, filtreler); }} disabled={sayfaNo === 0}
                    style={{ ...btn("#5C3D2E"), padding: "6px 12px", fontSize: 12, opacity: sayfaNo === 0 ? 0.4 : 1 }}>« İlk</button>
                  <button onClick={() => { const p = sayfaNo - 1; setSayfaNo(p); urunleriYukle(p, aramaMetni, filtreler); }} disabled={sayfaNo === 0}
                    style={{ ...btn("#5C3D2E"), padding: "6px 12px", fontSize: 12, opacity: sayfaNo === 0 ? 0.4 : 1 }}>← Önceki</button>
                  <button onClick={() => { const p = sayfaNo + 1; setSayfaNo(p); urunleriYukle(p, aramaMetni, filtreler); }} disabled={sayfaNo >= toplamSayfa - 1}
                    style={{ ...btn("#5C3D2E"), padding: "6px 12px", fontSize: 12, opacity: sayfaNo >= toplamSayfa - 1 ? 0.4 : 1 }}>Sonraki →</button>
                  <button onClick={() => { const p = toplamSayfa - 1; setSayfaNo(p); urunleriYukle(p, aramaMetni, filtreler); }} disabled={sayfaNo >= toplamSayfa - 1}
                    style={{ ...btn("#5C3D2E"), padding: "6px 12px", fontSize: 12, opacity: sayfaNo >= toplamSayfa - 1 ? 0.4 : 1 }}>Son »</button>
                </div>
              </div>
                    ...
                    {urunler.length === 0 ? (
                      ...
                      <tr key={urun.id} style={{ borderBottom: "1px solid #F5EFE8", background: seciliUrunler.includes(urun.id) ? "#FFF8F4" : "white" }}
                        ...
                        <td style={{ padding: "8px 10px", maxWidth: 220 }}>
                          <div style={{ fontWeight: 600, color: "#2C1A0E", lineHeight: 1.3 }}>{urun.ad?.substring(0, 48)}{urun.ad?.length > 48 ? "…" : ""}</div>
                          <div style={{ fontSize: 10, opacity: 0.35, marginTop: 2 }}>ID:{urun.id}</div>
                        </td>

                        {/* INLINE FİYAT DÜZENLEME */}
                        <td style={{ padding: "8px 10px", whiteSpace: "nowrap" }}>
                          {inlineEdit?.id === urun.id && inlineEdit.alan === "fiyat" ? (
                            <div style={{ display: "flex", gap: 4 }}>
                              <input type="number" step="0.01" value={inlineEdit.deger}
                                onChange={e => setInlineEdit({ ...inlineEdit, deger: e.target.value })}
                                onKeyDown={e => { if (e.key === "Enter") inlineKaydet(); if (e.key === "Escape") setInlineEdit(null); }}
                                autoFocus style={{ width: 80, padding: "4px 6px", border: "2px solid #E8845A", borderRadius: 6, fontSize: 12, outline: "none" }} />
                            ...
                            <span onClick={() => setInlineEdit({ id: urun.id, alan: "fiyat", deger: String(urun.fiyat) })}
                              style={{ fontWeight: 700, color: "#5C3D2E", cursor: "pointer", borderBottom: "1px dashed #ccc", paddingBottom: 1 }}
                              title="Hızlı düzenle için tıkla">
                              ₺{parseFloat(urun.fiyat).toFixed(2)}
                            </span>
                          )}
                        </td>

                        {/* INLINE İNDİRİMLİ FİYAT */}
                        <td style={{ padding: "8px 10px", whiteSpace: "nowrap" }}>
                          {inlineEdit?.id === urun.id && inlineEdit.alan === "indirimli_fiyat" ? (
                            ...
                            <span onClick={() => setInlineEdit({ id: urun.id, alan: "indirimli_fiyat", deger: String(urun.indirimli_fiyat || "") })}
                              style={{ fontWeight: 700, color: urun.indirimli_fiyat ? "#E8845A" : "#ccc", cursor: "pointer", borderBottom: "1px dashed #ccc", paddingBottom: 1 }}
                              title="Hızlı düzenle için tıkla">
                              {urun.indirimli_fiyat ? `₺${parseFloat(urun.indirimli_fiyat).toFixed(2)}` : "—"}
                            </span>
                          )}
                        </td>
                        ...
                        <td style={{ padding: "8px 10px" }}>
                          {inlineEdit?.id === urun.id && inlineEdit.alan === "stok" ? (
                            <div style={{ display: "flex", gap: 4 }}>
                              <input type="number" value={inlineEdit.deger}
                                onChange={e => setInlineEdit({ ...inlineEdit, deger: e.target.value })}
                                onKeyDown={e => { if (e.key === "Enter") inlineKaydet(); if (e.key === "Escape") setInlineEdit(null); }}
                                autoFocus style={{ width: 60, padding: "4px 6px", border: "2px solid #E8845A", borderRadius: 6, fontSize: 12, outline: "none" }} />
                              <button onClick={inlineKaydet} style={{ background: "#E8845A", color: "white", border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 11, cursor: "pointer" }}>✓</button>
                              <button onClick={() => setInlineEdit(null)} style={{ background: "#eee", border: "none", borderRadius: 6, padding: "4px 6px", fontSize: 11, cursor: "pointer" }}>✕</button>
                            </div>
                          ) : (
                        ...
                        <td style={{ padding: "8px 10px", fontSize: 12, color: "#5C3D2E", opacity: 0.65 }}>{urun.kategoriler?.ad || "—"}</td>
                        <td style={{ padding: "8px 10px", fontSize: 12, color: "#5C3D2E", opacity: 0.65 }}>{urun.markalar?.ad || "—"}</td>
                        <td style={{ padding: "8px 10px" }}>
                          {urun.etiket ? <span style={{ background: "#5C3D2E", color: "white", padding: "2px 8px", borderRadius: 50, fontSize: 10, fontWeight: 700 }}>{urun.etiket}</span> : null}
                        </td>
                        <td style={{ padding: "8px 10px" }}>
                          <button onClick={() => aktifToggle(urun.id, urun.aktif)}
                            style={{ background: urun.aktif ? "#E8F5E9" : "#FFEBEE", color: urun.aktif ? "#2E7D32" : "#C62828", border: "none", padding: "3px 10px", borderRadius: 50, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                        ...
                        <td style={{ padding: "8px 10px" }}>
                          <div style={{ display: "flex", gap: 5 }}>
                            <button onClick={() => setDuzenleUrun({ ...urun })}
                              style={{ background: "#FDF6EE", border: "2px solid #E8D5B7", borderRadius: 8, padding: "5px 10px", fontSize: 12, cursor: "pointer", fontWeight: 600, color: "#5C3D2E", whiteSpace: "nowrap" }}>
                              ✏️ Düzenle
                            </button>
                            <button onClick={() => urunSil(urun.id)}
                              style={{ background: "#FFEBEE", border: "none", borderRadius: 8, padding: "5px 9px", fontSize: 13, cursor: "pointer", color: "#C62828" }}>
                              🗑️
                            </button>
                          </div>
                        </td>
            ...
            <div style={{ marginTop: 8, fontSize: 11, color: "#5C3D2E", opacity: 0.45, textAlign: "center" }}>
              💡 Fiyat, indirimli fiyat ve stok hücrelerine tıklayarak hızlı düzenleme yapabilirsiniz
            </div>
          </div>
        )}
                ...
                <div key={s.id} style={{ background: "#FDF6EE", borderRadius: 16, padding: 18, marginBottom: 14, border: "1px solid #E8D5B7" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontFamily: "Georgia,serif", fontSize: 16, fontWeight: 700, color: "#5C3D2E" }}>#{s.siparis_no}</span>
                      <span style={{ fontSize: 12, opacity: 0.5 }}>{new Date(s.created_at).toLocaleString("tr-TR", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
                      <span style={{ background: s.odeme_yontemi === "kredi_karti" ? "#E3F2FD" : "#E8F5E9", color: s.odeme_yontemi === "kredi_karti" ? "#1565C0" : "#2E7D32", padding: "2px 9px", borderRadius: 50, fontSize: 11, fontWeight: 700 }}>
                        {s.odeme_yontemi === "kredi_karti" ? "💳 Kart" : "🏦 Havale"}
                      </span>
                    </div>
                    <div style={{ fontFamily: "Georgia,serif", fontSize: 20, fontWeight: 700, color: "#E8845A" }}>₺{parseFloat(s.toplam || 0).toFixed(2)}</div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 12 }}>
                    <div style={{ background: "white", borderRadius: 10, padding: "10px 14px" }}>
                      <div style={{ fontSize: 10, fontWeight: 700, opacity: 0.5, marginBottom: 4, textTransform: "uppercase" }}>Müşteri</div>
                      <div style={{ fontSize: 13, fontWeight: 700 }}>{s.ad} {s.soyad}</div>
                      <div style={{ fontSize: 11, opacity: 0.6 }}>{s.email}</div>
                      <div style={{ fontSize: 11, opacity: 0.6 }}>{s.telefon}</div>
                    </div>
                    <div style={{ background: "white", borderRadius: 10, padding: "10px 14px" }}>
                      <div style={{ fontSize: 10, fontWeight: 700, opacity: 0.5, marginBottom: 4, textTransform: "uppercase" }}>Teslimat Adresi</div>
                      <div style={{ fontSize: 12 }}>{s.adres}</div>
                      <div style={{ fontSize: 12, fontWeight: 600, marginTop: 2 }}>{s.sehir}</div>
                    </div>
                    <div style={{ background: "white", borderRadius: 10, padding: "10px 14px" }}>
                      <div style={{ fontSize: 10, fontWeight: 700, opacity: 0.5, marginBottom: 4, textTransform: "uppercase" }}>Kargo Takip</div>
                      <input type="text" autoComplete="off" placeholder="Takip numarası girin..."
                        defaultValue={s.kargo_takip || ""}
                        onBlur={async e => { if (e.target.value !== s.kargo_takip) { await supabase.from("siparisler").update({ kargo_takip: e.target.value }).eq("id", s.id); goster("✅ Takip no kaydedildi"); } }}
                        style={{ width: "100%", padding: "6px 10px", border: "2px solid #E8D5B7", borderRadius: 8, fontSize: 12, outline: "none", fontFamily: "inherit", boxSizing: "border-box" as const }} />
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    <select value={s.durum || "beklemede"} onChange={async e => { await supabase.from("siparisler").update({ durum: e.target.value }).eq("id", s.id); siparisleriYukle(siparisDurumFiltre); goster("✅ Sipariş durumu güncellendi"); }}
                      style={{ ...fltSelect }}>
                    ...
                    <select value={s.odeme_durumu || "beklemede"} onChange={async e => { await supabase.from("siparisler").update({ odeme_durumu: e.target.value }).eq("id", s.id); siparisleriYukle(siparisDurumFiltre); goster("✅ Ödeme durumu güncellendi"); }}
                      style={{ ...fltSelect }}>
                    ...
                    <button onClick={() => {
                      ...
                      if (!w) return;
                      w.document.write(`<html><head><title>Paketleme Fişi #${s.siparis_no}</title><style>body{font-family:Arial;padding:20px;max-width:420px}h2{border-bottom:2px solid #333;padding-bottom:8px}.row{display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px dashed #eee;font-size:13px}.total{font-size:18px;font-weight:bold;color:#E8845A;margin-top:12px}@media print{button{display:none}}</style></head><body><h2>🐾 evemama.net — Paketleme Fişi</h2><div class="row"><b>Sipariş No</b><span>#${s.siparis_no}</span></div><div class="row"><b>Tarih</b><span>${new Date(s.created_at).toLocaleDateString("tr-TR")}</span></div><div class="row"><b>Müşteri</b><span>${s.ad} ${s.soyad}</span></div><div class="row"><b>E-posta</b><span>${s.email || "-"}</span></div><div class="row"><b>Adres</b><span>${s.adres || "-"}, ${s.sehir || ""}</span></div><div class="row"><b>Ödeme</b><span>${s.odeme_yontemi === "kredi_karti" ? "Kredi Kartı" : "Havale/EFT"}</span></div><div class="total">Toplam: ₺${parseFloat(s.toplam || 0).toFixed(2)}</div><br/><button onclick="window.print()">🖨️ Yazdır</button></body></html>`);
                      w.document.close();
            ...
            <div style={{ background: "white", borderRadius: 18, padding: 22, boxShadow: "0 4px 16px rgba(92,61,46,0.06)" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr style={{ background: "#FAF5EF" }}>
                  {["ID", "Kategori Adı", "Slug", "Üst Kategori", "Durum"].map(h => <th key={h} style={{ padding: "10px 12px", textAlign: "left", fontSize: 10, fontWeight: 700, color: "#5C3D2E", opacity: 0.5, textTransform: "uppercase" }}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {kategoriler.map(k => <tr key={k.id} style={{ borderBottom: "1px solid #F0E8E0" }}>
                    <td style={{ padding: "10px 12px", fontSize: 11, opacity: 0.4 }}>{k.id}</td>
                    ...
                    <td style={{ padding: "10px 12px" }}><span style={{ background: k.aktif ? "#E8F5E9" : "#FFEBEE", color: k.aktif ? "#2E7D32" : "#C62828", padding: "3px 9px", borderRadius: 50, fontSize: 11, fontWeight: 700 }}>{k.aktif ? "Aktif" : "Pasif"}</span></td>
                  </tr>)}
                </tbody>
              </table>
            </div>
        ...
        {aktifSayfa === "markalar" && (
          <div>
            ...
            <div style={{ background: "white", borderRadius: 18, padding: 22, boxShadow: "0 4px 16px rgba(92,61,46,0.06)" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr style={{ background: "#FAF5EF" }}>
                  {["ID", "Marka Adı", "Slug", "Durum"].map(h => <th key={h} style={{ padding: "10px 12px", textAlign: "left", fontSize: 10, fontWeight: 700, color: "#5C3D2E", opacity: 0.5, textTransform: "uppercase" }}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {markalar.map(m => <tr key={m.id} style={{ borderBottom: "1px solid #F0E8E0" }}>
                    <td style={{ padding: "10px 12px", fontSize: 11, opacity: 0.4 }}>{m.id}</td>
                    ...
                    <td style={{ padding: "10px 12px" }}><span style={{ background: m.aktif ? "#E8F5E9" : "#FFEBEE", color: m.aktif ? "#2E7D32" : "#C62828", padding: "3px 9px", borderRadius: 50, fontSize: 11, fontWeight: 700 }}>{m.aktif ? "Aktif" : "Pasif"}</span></td>
                  </tr>)}
                </tbody>
              </table>
            </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {kuponlar.map(k => <tr key={k.id} style={{ borderBottom: "1px solid #F0E8E0" }}>
                    ...
                    <td style={{ padding: "12px" }}><button onClick={async () => { await supabase.from("kuponlar").delete().eq("id", k.id); kuponlariYukle(); goster("✅ Silindi"); }} style={{ background: "#FFEBEE", border: "none", borderRadius: 8, padding: "5px 10px", fontSize: 13, cursor: "pointer", color: "#C62828" }}>🗑️</button></td>
                  </tr>)}
                </tbody>
              </table>
            </div>
              ...
              {kargoAyar ? (
                <>
                  <div style={{ background: "#FDF6EE", borderRadius: 12, padding: "12px 16px", marginBottom: 24, fontSize: 13, color: "#5C3D2E" }}>
                    <strong>Mevcut Ayarlar:</strong> Ücretsiz limit = ₺{kargoAyar.ucretsiz_limit ?? kargoAyar["ucretsiz limit"] ?? "?"} | Sabit ücret = ₺{kargoAyar.sabit_ucret ?? "?"}
                  </div>
                  ...
                  <div style={{ marginBottom: 24 }}>
                    ...
                    <input type="number" step="0.01" value={kargoAyar.sabit_ucret || ""}
                      onChange={e => setKargoAyar({ ...kargoAyar, sabit_ucret: e.target.value })} style={s} />
                  </div>
                ...
                <div style={{ textAlign: "center", padding: "40px 0", opacity: 0.5 }}>
                  ...
                  <div style={{ fontSize: 12, marginTop: 8, opacity: 0.7 }}>Supabase'de kargo_ayarlari tablosunu kontrol edin</div>
                </div>
              )}
            </div>
          </div>
