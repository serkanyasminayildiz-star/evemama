...
export default function KategoriSayfasi() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#FDF6EE", fontFamily: "sans-serif" }}>
      ...
      {/* Breadcrumb */}
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "16px 24px", fontSize: 13, color: "#5C3D2E", opacity: 0.6 }}>
        <a href="/" style={{ color: "#E8845A", textDecoration: "none" }}>Ana Sayfa</a>
        {kategori && <> / <span>{kategori.ad}</span></>}
      </div>

      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 24px 48px", display: "grid", gridTemplateColumns: "260px 1fr", gap: 24 }}>
        ...
        <div>
          ...
          {altKategoriler.length > 0 && (
            <div style={{ background: "white", borderRadius: 20, padding: "20px", marginBottom: 16, boxShadow: "0 4px 16px rgba(92,61,46,0.06)" }}>
              ...
              {altKategoriler.map((alt, i) => {
                ...
                return (
                  <div key={i} onClick={() => setSeciliAltKat(alt.slug)}
                    ...
                    <span>{alt.ad}</span>
                    <span style={{ opacity: 0.5, fontSize: 12 }}>{sayi}</span>
                  </div>
                );
              })}
            </div>
          ...
          {markalar.length > 0 && (
            <div style={{ background: "white", borderRadius: 20, padding: "20px", marginBottom: 16, boxShadow: "0 4px 16px rgba(92,61,46,0.06)" }}>
              ...
              {(markalar as string[]).map((m, i) => (
                <div key={i} onClick={() => setSeciliMarka(m)}
          ...
          <div style={{ background: "white", borderRadius: 20, padding: "20px", boxShadow: "0 4px 16px rgba(92,61,46,0.06)" }}>
            ...
            {[
              ...
              <div key={i} onClick={() => setSiralama(s.value)}
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <h1 style={{ fontFamily: "Georgia, serif", fontSize: 24, fontWeight: 700, color: "#5C3D2E" }}>
              {kategori?.ad}
              <span style={{ fontSize: 14, fontWeight: 400, opacity: 0.5, marginLeft: 8 }}>({filtrelenmis.length} ürün)</span>
            </h1>
          </div>
          ...
          {altKategoriler.length > 0 && (
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
              <button onClick={() => setSeciliAltKat("")}
                style={{ padding: "8px 16px", borderRadius: 50, border: `2px solid ${!seciliAltKat ? "#E8845A" : "#E8D5B7"}`, background: !seciliAltKat ? "#E8845A" : "white", color: !seciliAltKat ? "white" : "#5C3D2E", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                Tümü
              </button>
              {altKategoriler.map((alt, i) => (
          ...
          {toplamSayfa > 1 && (
            <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 32 }}>
              <button onClick={() => setSayfa(s => Math.max(1, s - 1))} disabled={sayfa === 1}
                style={{ padding: "10px 18px", border: "2px solid #E8D5B7", borderRadius: 12, background: "white", cursor: sayfa === 1 ? "not-allowed" : "pointer", color: "#5C3D2E", fontWeight: 600, opacity: sayfa === 1 ? 0.4 : 1 }}>← Önceki</button>
              ...
              <button onClick={() => setSayfa(s => Math.min(toplamSayfa, s + 1))} disabled={sayfa === toplamSayfa}
                style={{ padding: "10px 18px", border: "2px solid #E8D5B7", borderRadius: 12, background: "white", cursor: sayfa === toplamSayfa ? "not-allowed" : "pointer", color: "#5C3D2E", fontWeight: 600, opacity: sayfa === toplamSayfa ? 0.4 : 1 }}>Sonraki →</button>
            </div>
          )}
        </div>
      </div>
