...
// Öne Çıkanlar: admin'de "öncelikli" işaretli TÜM aktif ürünler. Ana sayfa
// slider'ı ve ürün sayfası önerileriyle AYNI kaynak (oncelikli bayrağı) —
// eski /kategori/en-cok-satanlar linki boş kategoriye gidiyordu (0 ürün).
// Kart görünümü /urunler // /kampanyalar ile birebir.
type Urun = {
  id: number;
  ad: string;
  slug: string;
...
function indirimYuzde(u: Urun): number {
  if (!u.indirimli_fiyat || u.fiyat <= 0 || u.indirimli_fiyat >= u.fiyat) return 0;
...
export default function OneCikanlar() {
  ...
  useEffect(() => {
    supabase
      ...
      .eq("oncelikli", true)
      .then(({ data, error }) => {
        if (error) console.error("[one-cikanlar] urunler:", error);
        const liste = ((data || []) as unknown as Urun[]).sort((a, b) => {
          const stokFark = (b.stok > 0 ? 1 : 0) - (a.stok > 0 ? 1 : 0); // stoktakiler öne
          if (stokFark !== 0) return stokFark;
          return indirimYuzde(b) - indirimYuzde(a); // sonra indirimliler öne
        });
        setUrunler(liste);
        setYukleniyor(false);
      });
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#FDF6EE", fontFamily: "sans-serif" }}>
      ...
      <style>{`
        .onecikan-wrap { max-width: 1200px; margin: 0 auto; padding: 0 24px 64px; }
        .onecikan-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 16px; }
        .header-onecikan { padding: 16px 48px; }
        @media (max-width: 1024px) {
          .onecikan-grid { grid-template-columns: repeat(3,1fr); }
        }
        @media (max-width: 768px) {
          .onecikan-wrap { padding: 0 14px 64px; }
          .onecikan-grid { grid-template-columns: repeat(2,1fr) !important; gap: 10px; }
          .header-onecikan { padding: 13px 16px !important; }
        }
      `}</style>

      {/* Header */}
      <header className="header-onecikan" style={{ background: "white", borderBottom: "1px solid #E8D5B7", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 100 }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "12px 16px", fontSize: 13, color: "#5C3D2E", opacity: 0.6 }}>
        <Link href="/" style={{ color: "#E8845A", textDecoration: "none" }}>Ana Sayfa</Link> / Öne Çıkanlar
      </div>

      <div className="onecikan-wrap">

        {/* Başlık şeridi */}
        <div style={{ background: "linear-gradient(135deg,#FFF3E0,#FFE0B2)", borderRadius: 20, padding: "26px 24px", marginBottom: 24, textAlign: "center" }}>
          <div style={{ fontSize: 34, marginBottom: 6 }}>⭐</div>
          <h1 style={{ fontFamily: "Georgia, serif", fontSize: 26, fontWeight: 700, color: "#5C3D2E", margin: 0 }}>Öne Çıkan Ürünler</h1>
          <p style={{ fontSize: 14, color: "#5C3D2E", opacity: 0.7, margin: "8px 0 0" }}>
            Sizin için seçtiklerimiz{!yukleniyor && urunler.length > 0 ? ` — ${urunler.length} ürün` : ""}
          </p>
        </div>
        ...
        {yukleniyor ? (
          ...
          <div style={{ textAlign: "center", padding: "60px 24px", background: "white", borderRadius: 24 }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>🐾</div>
            <div style={{ fontFamily: "Georgia, serif", fontSize: 18, color: "#5C3D2E", marginBottom: 8 }}>Şu anda öne çıkan ürün yok</div>
            <Link href="/urunler" style={{ background: "#E8845A", color: "white", padding: "12px 28px", borderRadius: 50, textDecoration: "none", fontWeight: 700, fontSize: 14, display: "inline-block", marginTop: 12 }}>Tüm Ürünlere Göz At →</Link>
          </div>
        ) : (
          <div className="onecikan-grid">
            {urunler.map(urun => {
              const yuzde = indirimYuzde(urun);
              return (
                <div key={urun.id} style={{ background: "white", borderRadius: 18, overflow: "hidden", transition: "transform .2s, box-shadow .2s" }}
                  ...
                  <div style={{ padding: "0 12px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <span style={{ fontFamily: "Georgia, serif", fontSize: 15, fontWeight: 700, color: yuzde > 0 ? "#E53935" : "#5C3D2E" }}>₺{(urun.indirimli_fiyat || urun.fiyat).toFixed(2)}</span>
                      {yuzde > 0 && <span style={{ fontSize: 10, color: "#5C3D2E", opacity: 0.4, textDecoration: "line-through", marginLeft: 4 }}>₺{urun.fiyat.toFixed(2)}</span>}
                    </div>
